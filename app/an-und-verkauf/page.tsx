...
export default function AnUndVerkaufPage() {
  ...
  // HCI-Integration - wird nur ausgeführt wenn Ankauf-Tab aktiv ist
  useEffect(() => {
    if (tab !== "ankauf") return
    const w = window as any

    // 1) Bevorzugt: offizielle Mount/Refresh API nutzen falls vorhanden
    if (w.hci?.mount && hciRef.current) {
      if (!(hciRef.current as any)._mounted) {
        console.log('HCI Container wird gemountet...')
        w.hci.mount(hciRef.current)
        ;(hciRef.current as any)._mounted = true
      } else {
        console.log('HCI Container wird aktualisiert...')
        w.hci.refresh?.()
      }
      return
    }

    // 2) Fallback: Script erneut laden -> triggert Re-Scan
    console.log('HCI nicht verfügbar, lade Script neu...')
    const s = document.createElement("script")
    s.src = "https://www.autoscout24.ch/assets/hci/v2/hci.current.js?v=" + Date.now()
    s.async = true
  ...
  return (
    <>
      {/* Script EINMAL global laden */}
      <Script
        id="autoscout24-hci"
        src="https://www.autoscout24.ch/assets/hci/v2/hci.current.js"
